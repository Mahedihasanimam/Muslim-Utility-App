...
// import tw from 'twrnc';
// import { hadiths } from '../../assets/hadiths'; // <-- নিশ্চিত করুন ফাইলটি এখানে আছে

// // --- নোটিফিকেশন হ্যান্ডলার (সলভ্ করা) ---
...
//   const [nextPrayerInfo, setNextPrayerInfo] = useState<{ name: string; time: string; index: number } | null>(null);
//   const [ramadanDaysLeft, setRamadanDaysLeft] = useState<number>(0);
//   const [isLoading, setIsLoading] = useState(true); // <-- API লোডিং
//   const [errorMsg, setErrorMsg] = useState<string | null>(null);
//   const [locationName, setLocationName] = useState('Dhaka, Bangladesh');
...
//   const [isPrayerLogLoading, setIsPrayerLogLoading] = useState(true); // <-- AsyncStorage লোডিং
...
//   // --- হাদিস ফাংশন [সলভ্: অধিক মজবুত করা হয়েছে] ---
//   const handleSelectMood = (mood: string) => {
...
//     // যদি ওই ক্যাটাগরিতে হাদিস না পাওয়া যায়, তবে সব হাদিস থেকে একটি নিন
...
//         setErrorMsg(e.message);
//       } finally {
//         setIsLoading(false); // শুধু API লোডিং শেষ
//       }
//     };
//     loadInitialData();
//   }, []);

//   // --- লগ লোড এবং নোটিফিকেশন শিডিউল ---
//   useEffect(() => {
//     // prayerData লোড না হলে বা এরর থাকলে লগ লোড করার দরকার নেই
//     if (!prayerData) {
//       // যদি prayerData লোড না হয় (যেমন API ফেল করে), তবুও লগ লোডিং বন্ধ করতে হবে
//       if (!isLoading) { // নিশ্চিত করুন API লোড চেষ্টা শেষ করেছে
...
//       finally {
//         setIsPrayerLogLoading(false); // AsyncStorage লোডিং শেষ
//       }
//     };
...
//   }, [prayerData, isLoading]); // <-- prayerData অথবা isLoading পরিবর্তন হলে চলবে

//   // --- [সলভ্ ১: মোডাল দেখানোর নতুন useEffect] ---
//   useEffect(() => {
//     // যখন *উভয়* লোডিং শেষ হবে
//     if (!isLoading && !isPrayerLogLoading) {
//       // এবং কোনো বড় এরর না থাকলে
//       if (!errorMsg) {
//         setMoodModalVisible(true);
//       }
//     }
...
//     // prayerData লোড না হলে লগ সেভ করা যাবে না
//     if (!prayerData) {
...
//   if (isLoading || isPrayerLogLoading) return ( // <-- দুটি লোডিং স্টেটই চেক করা হচ্ছে
...
//         onRequestClose={() => {
//           if (!currentHadith) handleSelectMood('guidance'); // <-- ডিফল্ট হাদিস লোড
//           setMoodModalVisible(false);
//         }}
//       >
//         <TouchableOpacity
